...
protected:
    ...
    void attackAnimation(auto& game) {
        ...
        if (pam == 5) {
            ...
            game.renderer->AnimationWeaponSprite.setTextureRect(IntRect(80 * (pam - 1) + pam, 76 * pdir + pdir + 1, 80, 76)); //80, 76 - width, height
            game.renderer->AnimationWeaponSprite.setPosition(game.player->getX() - 16, game.player->getY() - 4);
        }
    }
    ...
    void shieldAnimation(auto& game) {
        auto& pdir = game.player->dir;
        int n = pdir / 2 * (pdir - 1);
        game.renderer->AnimationShieldSprite.setTextureRect(IntRect(47 * n + n + 1, 1, 47, 28));
        game.renderer->AnimationShieldSprite.setPosition(game.player->getX(), game.player->getY() + 30);
    }
    ...
    void dashAnimation(auto& game) {
        ...
        if (game.player->dash_timer > 0) {
            ...
            game.renderer->AnimationDashSprite.setTextureRect(IntRect(94 * pdir + pdir + 1, 1, 94, 108));
            game.renderer->AnimationDashSprite.setPosition(game.player->getX() - 26, game.player->getY() - 22);
        }
    }
};
