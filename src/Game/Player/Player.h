...
public:
	    ...
	    for (int i = p_cords[1] / 64; i < (p_cords[1]+ h) / 64; i++) {
	        for (int j = p_cords[0] / 64; j < (p_cords[0] + w) / 64; j++) {
				for (int l = 0; l < v_NPC.size(); l++) {
					if (p_cords[0] < v_NPC[l].getX() + 54 &&
					    p_cords[0] + 50 > v_NPC[l].getX() &&
					    p_cords[1] < v_NPC[l].getY() + 62 &&
					    p_cords[1] + 62 > v_NPC[l].getY())
					{
    ...
    void update(float time, auto& map_manager, auto& v_NPC, auto& v_monsters, auto& game) {
		switch (dir) {
        ...
        keys(time, v_NPC, v_monsters, game);
	}
