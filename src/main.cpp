...
public:
    ...
    void createWindow() { window = std::make_shared<RenderWindow>(VideoMode(window_w, window_h), "Lord World"); }
    void viewReset() { view.reset(FloatRect(0, 0, window_w, window_h)); }
    ...
    void windowSetView() { window->setView(view); }
    void windowClear() { window->clear(); }
    void windowDisplay() { window->display(); }
